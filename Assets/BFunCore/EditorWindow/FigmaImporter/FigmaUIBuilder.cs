        ...
        public static async Task ImportFigmaUI(string url, string token, string imageSavePath, string targetCanvas)
        {
...
#if UNITY_EDITOR
            string[] guids = AssetDatabase.FindAssets("t:GUITextStyleSheet");
            if (guids.Length > 0)
            {
                string path = AssetDatabase.GUIDToAssetPath(guids[0]);
                GUITextStyleSheet styleSheet = AssetDatabase.LoadAssetAtPath<GUITextStyleSheet>(path);
                if (styleSheet != null && styleSheet.styles != null && styleSheet.styles.Length > 0)
                {
                    defaultFont = styleSheet.styles[0].font;
                }
            }
#endif
            ...
            try
            {
                ...
                if (response?.Document != null)
                {
                    ...
                    BuildUI(filteredDoc, defaultFont, spritesMap, response.Styles);
                }
            }
            catch (Exception e)
            {
