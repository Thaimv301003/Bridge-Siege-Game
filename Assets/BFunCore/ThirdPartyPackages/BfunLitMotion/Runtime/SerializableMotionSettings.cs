...
namespace Bfun.LitMotion
{
        ...
        where TOptions : unmanaged, IMotionOptions
    {
        ...
        {
            UpdateSchedulerFromEnum();
        }
#endif

        public void OnBeforeSerialize()
        {
            // FIX: Chỉ cập nhật Enum từ Scheduler nếu Scheduler thực sự tồn tại (được set qua code).
            // Nếu Scheduler là null (chưa chạy game), KHÔNG ĐƯỢC reset schedulerType về Default.
            if (scheduler != null)
            {
                SetSchedulerToEnum(scheduler);
            }
        }
        ...
        void SetSchedulerToEnum(IMotionScheduler value)
        {
            if (value == null) return; // Không làm gì nếu null để bảo toàn giá trị Inspector

            if (value == MotionScheduler.Update) schedulerType = SchedulerType.Update;
